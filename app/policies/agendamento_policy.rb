class AgendamentoPolicy
    ...
    attr_reader :usuario, :agendamento
  
    def initialize(usuario, agendamento)
      @usuario = usuario  
    ...
    def new?
        coordenadorCurso? || professorTurma? || representanteTurma? || monitorDiscipina?
    end
    ...
    def monitorDiscipina?
        Usuario.cargo_usuarios[@usuario.cargo_usuario] == 5
    end

    def criouAgendamento?
        if @usuario then
